...
pub const DECISION_GRADE_METRICS: &[MetricContract] = &[
    ...
    MetricContract::new("start.resume_to_first_stdout_ms", MetricEndpoint::SnapshotRestoreStart, MetricEndpoint::FirstStdoutByte, LifecycleClass::Resumed, WorkloadClass::TinyExec, RuntimeProfile::FastAgent, "snapshot restore, readiness, exec request, stdout wait", "template build, pool lease acquisition, cleanup", "firkin-runtime/firkin-single-node", FAST_PATH_POLICY, FOCUSED),
    ...
    MetricContract::new("exec.command_start_ms", MetricEndpoint::ExecRequestSent, MetricEndpoint::ProcessStarted, LifecycleClass::Hot, WorkloadClass::TinyExec, RuntimeProfile::FastAgent, "exec RPC dispatch through guest process start", "pool lease, readiness, stdout wait, process exit, cleanup", "firkin-vminitd-client/firkin-runtime", FAST_PATH_POLICY, FOCUSED),
    MetricContract::new("exec.first_stdout_byte_ms", MetricEndpoint::ExecRequestSent, MetricEndpoint::FirstStdoutByte, LifecycleClass::Hot, WorkloadClass::TinyExec, RuntimeProfile::FastAgent, "exec RPC dispatch, process start, stdout wait", "pool lease, readiness, process exit, cleanup", "firkin-vminitd-client/firkin-runtime", FAST_PATH_POLICY, FOCUSED),
    ...
    MetricContract::new("density.max_active_before_hot_to_first_stdout_p95_doubles", MetricEndpoint::PoolLeaseAcquired, MetricEndpoint::FirstStdoutByte, LifecycleClass::Hot, WorkloadClass::ConcurrentCreate, RuntimeProfile::Density, "concurrency sweep of hot-to-first-stdout p95", "other workload p95s, cold/warm/resumed lifecycles", "firkin-benchmark/firkin-runtime", SLOW_PATH_POLICY, FOCUSED),
