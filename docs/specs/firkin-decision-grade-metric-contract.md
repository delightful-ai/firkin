...
| `start.resume_to_first_stdout_ms` | `SnapshotRestoreStart` | `FirstStdoutByte` | `resumed` | `tiny_exec` | `fast_agent` | snapshot restore, readiness, exec request, stdout wait | template build, pool lease acquisition, cleanup | `firkin-runtime/firkin-single-node` | 100 | 500 |
...
| `exec.command_start_ms` | `ExecRequestSent` | `ProcessStarted` | `hot` | `tiny_exec` | `fast_agent` | exec RPC dispatch through guest process start | pool lease, readiness, stdout wait, process exit, cleanup | `firkin-vminitd-client/firkin-runtime` | 100 | 500 |
| `exec.first_stdout_byte_ms` | `ExecRequestSent` | `FirstStdoutByte` | `hot` | `tiny_exec` | `fast_agent` | exec RPC dispatch, process start, stdout wait | pool lease, readiness, process exit, cleanup | `firkin-vminitd-client/firkin-runtime` | 100 | 500 |
...
| `density.max_active_before_hot_to_first_stdout_p95_doubles` | `PoolLeaseAcquired` | `FirstStdoutByte` | `hot` | `concurrent_create` | `density` | concurrency sweep of hot-to-first-stdout p95 | other workload p95s, cold/warm/resumed lifecycles | `firkin-benchmark/firkin-runtime` | 30 | 500 |
